...
It is possible to have the parser apply a limited number of transformations to the value used to replace a given tag. Currently, the transformations are limited to 'upper', 'lower' and 'title' and these make the value all lowercase, all uppercase or titlecase respectively. You can specify a transformation by appending it to the end of the tag like this @{name|upper}@ or @{page.name|title}@.
...
If you are formatting something that needs the substitutions to be URL encoded just...

bc. $parser->set('encode', true);

p. ...and the parser will encode as it goes. By default, the parser will not perform any encoding.
