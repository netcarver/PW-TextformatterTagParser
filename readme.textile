...
It is possible to have the parser apply a number of transformations to the value used to replace a given tag. You can specify a transformation by appending it to the end of the tag like this @{name>upper}@ or @{page.name>title}@.

You can chain transformations so: @{title>lower>url}@ which will lowercase the title field value and then URL encode it.

|_. Transformation |_. Description |
| lower | Runs the value through @mb_strtolower()@ |
| upper | Runs the value through @mb_strtoupper()@ |
| title | Runs the value through @mb_caseconvert( , MB_CASE_TITLE, )@ |
...
h3. URL encode substitutions:

If you are formatting something that needs all the substitutions to be URL encoded just...

bc. $parser->set('encode', true);

p. ...and the parser will encode as it goes. By default, the parser will only perform url encoding if your tag has the 'url' transformation specified on it, like this; @{page.title|url}@.

h3. Putting it together:

You can format as many blocks as you want with a given parser & context...

bc.. $body = "Hi {firstname} and welcome to our service. You can login using your username {name} and password.";
$subject = "Welcome {name}";
$parser = wire()->modules->get("TextformatterTagParser");
$parser
    ->set('context', $user)
    ->format($subject)      // make $subject specific to this user
    ->format($body)         // make $body specific to them too
    ;
