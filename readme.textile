...
It is possible to have the parser apply a limited number of transformations to the value used to replace a given tag. Currently, the transformations are limited to 'upper', 'lower', 'title' & 'url' and these make the value all lowercase, all uppercase, all title-case or urlencoded respectively. You can specify a transformation by appending it to the end of the tag like this @{name|upper}@ or @{page.name|title}@.
